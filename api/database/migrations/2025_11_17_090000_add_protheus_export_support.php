...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('purchase_quotes', function (Blueprint $table) {
            $table->string('protheus_export_status', 30)
                ->default('idle')
                ->after('requires_response');
            $table->timestamp('protheus_exported_at')
                ->nullable()
                ->after('protheus_export_status');
        });
        ...
        Schema::table('purchase_quote_suppliers', function (Blueprint $table) {
            ...
            $table->timestamp('protheus_exported_at')
                ->nullable()
                ->after('protheus_export_status');
            $table->string('protheus_order_number', 60)
                ->nullable()
                ->after('protheus_exported_at');
            $table->unsignedInteger('protheus_export_attempts')
                ->default(0)
                ->after('protheus_order_number');
            $table->text('protheus_last_error')
                ->nullable()
                ->after('protheus_export_attempts');

            $table->index('protheus_export_status', 'pqs_protheus_export_status_idx');
        });

        Schema::table('purchase_quote_items', function (Blueprint $table) {
            $table->index('selected_supplier_id', 'pqi_selected_supplier_idx');
        });
...
SQL);
    ...
    public function down(): void
    {
        ...
        Schema::table('purchase_quote_items', function (Blueprint $table) {
            $table->dropIndex('pqi_selected_supplier_idx');
        });

        Schema::table('purchase_quote_suppliers', function (Blueprint $table) {
            $table->dropIndex('pqs_protheus_export_status_idx');
            $table->dropColumn([
