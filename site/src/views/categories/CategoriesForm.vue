	...
	methods: {
		...
		getCategory() {
			...
			if (this.route.params?.id) {
				this.category = ref(null);
				this.loading = true;
				this.categoriesService.get(this.route.params.id)
				.then((response) => {
		...
		save() {
			...
			.then((response) => {
				...
				this.toast.add({
					...
					detail: this.category?.id ? 'Dados alterados com sucesso!' : 'Dados inseridos com sucesso!',
					life: 3000
				});
		...
		addCityBeforeSave(city) {
			// this.category.cities.push(city);
			this.changeLoading();
		}
	},
	computed: {
		title() {
			return this.route.params?.id ? 'Editar Categoria' : 'Criar Categoria';
		}
	},
	mounted() {
		this.getCategory();
		...
		<template #content>
			<div class="formgrid grid">
				<div class="field col-12 md:col-12 lg:col-12 xl:col-12">
					...
					<InputText :modelValue="category?.name" v-model="category.name" id="name" type="text" class="w-full p-inputtext-sm" :class="{ 'p-invalid': errors?.name }" />
			...
			<div class="formgrid grid">
				<div class="field col-12 md:col-12 lg:col-12 xl:col-12">
					...
					<InputText :modelValue="category?.description" v-model="category.description" id="description" type="text" class="w-full p-inputtext-sm" :class="{ 'p-invalid': errors?.description }" />
