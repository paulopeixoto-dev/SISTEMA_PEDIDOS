<template>
  <div class="card relatorio-container">
    <div class="mb-4 flex justify-content-between align-items-center flex-wrap gap-3">
      <div>
        <h4 class="m-0 text-900">Relatório de Cotação</h4>
        <p class="m-0 text-600">
          Visualize as cotações com valores consolidados, status e fornecedor selecionado.
        </p>
      </div>
    ...
    >
      ...
      <Column field="numero" header="Cotação" sortable>
        <template #body="slotProps">
          <div class="flex flex-column">
            <span class="font-semibold text-900">{{ slotProps.data.numero }}</span>
            <small class="text-600">Status: {{ slotProps.data.status.label }}</small>
          </div>
        </template>
      </Column>
      ...
      <Column field="fornecedor_selecionado" header="Fornecedor Selecionado">
        <template #body="slotProps">
          <div v-if="slotProps.data.fornecedor_selecionado" class="flex flex-column">
            <span class="font-semibold text-900">
              {{ slotProps.data.fornecedor_selecionado.nome }}
            </span>
            <small class="text-600">
              {{ slotProps.data.fornecedor_selecionado.valor_total_formatado }}
            </small>
          </div>
      ...
      <Column field="data_finalizada_formatado" header="Finalizada em" sortable />

      <template #expansion="slotProps">
        <div class="expansion-container">
          <h5 class="text-900 mb-3">Detalhes da cotação</h5>
          <div class="grid">
            <div class="col-12 md:col-6">
              <p class="m-0 text-600">Centro de Custo Principal</p>
              <p class="m-0 text-900 font-medium">
                {{ slotProps.data.centro_custo_principal?.codigo ?? 'Não informado' }}
              </p>
              <small class="text-600">
                {{ slotProps.data.centro_custo_principal?.descricao ?? 'Sem descrição' }}
              </small>
            </div>
            <div class="col-12 md:col-6">
              <p class="m-0 text-600">Fornecedor Selecionado</p>
              <p class="m-0 text-900 font-medium">
                {{ slotProps.data.fornecedor_selecionado?.nome ?? 'Não informado' }}
              </p>
              <small class="text-600">
                {{
                  slotProps.data.fornecedor_selecionado?.valor_total_formatado ?? 'Sem valor informado'
                }}
              </small>
            </div>
          </div>
...
const totais = computed(() => {
  const valorTotal = cotacoes.value.reduce((sum, item) => sum + (item.valor_total ?? 0), 0);
  const totalItens = cotacoes.value.reduce((sum, item) => sum + (item.total_itens ?? 0), 0);
...
const carregarDados = async () => {
  try {
    ...
    const { data } = await RelatorioService.relatorioCotacoes(montarParametros());
