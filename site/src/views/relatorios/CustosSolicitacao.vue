<template>
  <div class="card relatorio-container">
    <div class="mb-4 flex justify-content-between align-items-center flex-wrap gap-3">
      <div>
        <h4 class="m-0 text-900">Custos por Solicitação</h4>
        <p class="m-0 text-600">
          Lista as cotações com seus respectivos valores e fornecedores participantes.
        </p>
      </div>
    ...
    >
      ...
      <Column field="numero" header="Solicitação" sortable />

      <Column field="status.label" header="Status" sortable>
        <template #body="slotProps">
          <Tag :value="slotProps.data.status.label" />
        </template>
      </Column>
      ...
      <template #expansion="slotProps">
        <div class="expansion-container">
          <h5 class="text-900 mb-3">Fornecedores participantes</h5>
          <DataTable
            :value="slotProps.data.fornecedores"
            responsiveLayout="scroll"
          ...
          >
            ...
            <Column field="nome" header="Fornecedor" />
...
const totais = computed(() => {
  const valorTotal = solicitacoes.value.reduce((sum, item) => sum + (item.valor_total ?? 0), 0);
  const totalItens = solicitacoes.value.reduce((sum, item) => sum + (item.total_itens ?? 0), 0);
...
const carregarDados = async () => {
  try {
    ...
    const { data } = await RelatorioService.custosPorSolicitacao(montarParametros());
